# ================================================================================================================================
# < 파이썬 스코프와 네임스페이스(Namespaces) >
# 파이썬에서 변수의 유효 범위를 계산할 때 네임스페이스(namespace)를 기반으로 계산한다.
# 네임스페이스는 변수를 구분할 수 있는 범위를 의미하며, 네임스페이스에는 하나의 이름이 하나의 변수만 가리키게 된다.
# 파이썬의 네임스페이스는 built-in, global, enclosed, local로 나뉘어지며, 각 네임스페이스에 따라 변수의 유효범위가 달라지게 된다.
# 
# 1. global 네임스페이스 : 모듈별로 존재하며, 모듈 전체에 통용되는 이름을 사용한다. 파일 단위의 모듈 안에서 사용될 수 있다.
# 2. local 네임스페이스 : 함수 및 메소드 별로 존재하며, 함수 내의 지역 변수들이 소속된다.
# 3. built-in 네임스페이스 : 기본 내장 함수 및 기본 예외들의 이름을 저장하는 곳
# 4. enclosed 네임스페이스 : nested function 구조에서 내부 함수에서 외부 함수의 변수에 접근할 수 있다.
# 5. 어떤 의미에서 객체의 어트리뷰트 집합도 네임스페이스를 형성한다.
# 
# 스코프는 네임스페이스를 직접 엑세스할 수 있는 파이썬 프로그램의 텍스트 적인 영억이다.
# 스코프가 정적으로(statically) 결정됨에도 불구하고, 동적으로(dynamically) 사용됩니다. 실행 중 언제든지 네임스페이스를 직접 엑세스 가능한 세 개 또는 네 개의 중첩된 스코프가 있습니다.
# 🔴 LEGB 규칙 : 변수를 확인할 때 Local -> Enclosed -> Global -> Built-in 순서로 네임스페이스를 확인하는 것을 의미한다.
...
# 네임스페이스 예제1
my_cat = "GLOBAL 땅콩"
print(globals()['my_cat']) # GLOBAL 땅콩
def printCat():
    my_cat = "LOCAL 땅콩"
    print(locals())

printCat()  # {'my_cat': 'LOCAL 땅콩'}

print(dir(__builtins__)) # 빌트인 네임스페이스
